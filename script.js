...
const addGameBoxes = (gameList) => {
    for(i in gameList) {
        ...
        if(gameList[i].background_image != null)
            image.src = gameList[i].background_image;
        else 
            image.src = 'https://static.vecteezy.com/system/resources/previews/004/141/669/original/no-photo-or-blank-image-icon-loading-images-or-missing-image-mark-image-not-available-or-image-coming-soon-sign-simple-nature-silhouette-in-frame-isolated-illustration-vector.jpg';
        ...
        if(gameList[i].metacritic != null)
            metacritic.innerText = `Metacritic score: ${gameList[i].metacritic}`;
        else 
            metacritic.innerText = `Metacritic score: -`;
...
const showBestGames = async (APIkey) => {
    ...
    let response = await fetch(`https://rawg-video-games-database.p.rapidapi.com/games?key=${APIkey}&ordering=-metacritic`, options);
...
const searchGames = async (APIkey, query) => {
    ...
    let response = await fetch(`https://rawg-video-games-database.p.rapidapi.com/games?key=${APIkey}&search=${query}&search_precise=true`, options);
    response = await response.json();
    response.results.forEach(element => {
        if(element.reviews_count > 10)
            gamesFound.push(element)
    });
